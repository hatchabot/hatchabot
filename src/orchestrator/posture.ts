...
export function computePosture(store: Store, input: PostureInput): PostureReport {
  ...
  if (isHostOwner) {
    ...
    install.push(
      process.env.AGENTCLAW_ALLOW_OWNER_HEADER === '1'
        ? {
            key: 'owner-header',
            level: 'critical',
            title: 'AGENTCLAW_ALLOW_OWNER_HEADER is ON',
            detail: 'The x-agentclaw-owner header forges ownership. Never set this in production — unset it.',
          }
        : { key: 'owner-header', level: 'ok', title: 'Owner-header spoof disabled', detail: 'x-agentclaw-owner is not honored.' },
    );
    ...
    const cap = envNum('AGENTCLAW_MAX_AGENTS_PER_ACCOUNT', 0);
    install.push(
      cap > 0
        ...
        : {
            ...
            detail: 'AGENTCLAW_MAX_AGENTS_PER_ACCOUNT is unset — one account can spawn unlimited agents. Set it before adding members.',
          },
    );
  ...
  return {
    install,
    agents,
    limits: { agentCap: envNum('AGENTCLAW_MAX_AGENTS_PER_ACCOUNT', 0), liveAgents, diskWarnGB: envNum('AGENTCLAW_AGENT_DISK_WARN_GB', 10) },
  };
}
