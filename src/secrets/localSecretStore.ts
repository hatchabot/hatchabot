...
/**
 * Envelope-free AES-256-GCM at rest, keyed by HATCHABOT_SECRET_KEY.
 *
...
export class LocalSecretStore implements SecretStore {
  ...
  /**
   * HATCHABOT_SECRET_KEY can be either 64 hex chars (used as the raw key) or
  ...
  static keyFromEnv(env = process.env): Buffer {
    const value = env.HATCHABOT_SECRET_KEY;
    if (!value) {
      throw new Error(
        'HATCHABOT_SECRET_KEY is not set. Use any passphrase, or generate a key with:\n' +
