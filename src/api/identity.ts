...
export class IdentityVerifier {
  ...
  constructor(config: IdentityConfig) {
    if (!config.projectId) {
      throw new Error('AGENTCLAW_GCP_PROJECT is required when AGENTCLAW_AUTH=identity');
    }
    this.#config = config;
  }
...
export function identityConfigFromEnv(env = process.env): IdentityConfig {
  return {
    projectId: env.AGENTCLAW_GCP_PROJECT ?? '',
    apiKey: env.AGENTCLAW_IDENTITY_API_KEY,
  };
}
