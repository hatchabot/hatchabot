 ...
 * source). Start it as a systemd unit — see deploy/hatchabot-mgmt-bot.service
 * and docs/control-interfaces.md.
 *
 * Required env:
 *   HATCHABOT_MGMT_BOT_TOKEN   BotFather token for the management bot
 *   HATCHABOT_MGMT_TOKEN       a cli-token (POST /v1/cli-tokens) — the bearer
 *   HATCHABOT_MGMT_ALLOWLIST   comma-separated Telegram user ids with FULL control (operators)
 * Optional:
 *   HATCHABOT_URL                    control plane base URL (default http://localhost:8080)
 *   HATCHABOT_MGMT_VIEWERS           comma-separated Telegram user ids with READ-ONLY access
 *                                    (query the fleet, but no mutations/mode/pause/join approvals)
 *   HATCHABOT_MGMT_OWNER             owner id for audit/proposer records (default "local")
 *   HATCHABOT_MGMT_ANTHROPIC_KEY     dedicated LLM credential — overrides the proxy path
...
// straight after `hatchabot mgmt-bot setup`. Under systemd the EnvironmentFile
...
const botToken = required('HATCHABOT_MGMT_BOT_TOKEN');
const apiToken = required('HATCHABOT_MGMT_TOKEN');
const baseUrl = process.env.HATCHABOT_URL ?? 'http://localhost:8080';
const ownerId = process.env.HATCHABOT_MGMT_OWNER ?? 'local';
...
const allowlist = parseIds(process.env.HATCHABOT_MGMT_ALLOWLIST);
const viewers = parseIds(process.env.HATCHABOT_MGMT_VIEWERS).filter((n) => !allowlist.includes(n));
if (allowlist.length === 0) {
  console.error('HATCHABOT_MGMT_ALLOWLIST is empty — set the Telegram id(s) allowed to control the fleet.');
  process.exit(1);
}
...
// HATCHABOT_MGMT_ANTHROPIC_KEY remains as an explicit override for a dedicated
...
const anthropicKey = process.env.HATCHABOT_MGMT_ANTHROPIC_KEY ?? process.env.ANTHROPIC_API_KEY;
const llm = anthropicKey
  ? new LlmAgent(new AnthropicChatModel(anthropicKey, process.env.HATCHABOT_MGMT_MODEL ?? 'claude-sonnet-5'), broker)
...
const pairingNotifier = createPairingNotifier(api, transport, allowlist, {
  intervalMs: Number(process.env.HATCHABOT_MGMT_PAIRING_POLL_MS) || 20_000,
...
const startHeartbeat = (botUsername: string) => {
  const beat = async () => {
    ...
    if (anthropicKey) {
      llmLabel = process.env.HATCHABOT_MGMT_MODEL ?? 'claude-sonnet-5';
    } else {
      try {
  ...
  onStart: (me) => {
    ...
    console.log(
      JSON.stringify({
        ...
        allowlisted: allowlist.length,
        mode: broker.readWrite ? 'read-write' : 'read-only',
        llm: anthropicKey ? (process.env.HATCHABOT_MGMT_MODEL ?? 'claude-sonnet-5') : 'via control-plane proxy',
      }),
    );
  },
});
