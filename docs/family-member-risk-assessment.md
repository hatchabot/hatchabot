...
Consolidated from a five-reviewer threat-model audit of AgentClaw v0.119.0.
...
- **B cannot mount your files.** Host-folder mounts are gated by `ownsLocalHost`,
  ...
  `~/.claude`, `~/.ssh`, `~/.config/agentclaw`, `/etc`, `/root`, `/`, etc.
...
plane); **no CPU limit**; and `AGENTCLAW_MAX_AGENTS_PER_ACCOUNT` **defaults to 0
...
- **Keep `AGENTCLAW_ALLOW_OWNER_HEADER` unset** in production (it's a full
...
1. **Run identity mode** (`AGENTCLAW_AUTH=identity`) and **confirm you own the
...
3. **Set `AGENTCLAW_MAX_AGENTS_PER_ACCOUNT`** to a sane number (e.g. 3–5).
4. **Add the member as a management VIEWER** (`AGENTCLAW_MGMT_VIEWERS`), never to
   `AGENTCLAW_MGMT_ALLOWLIST`. (The viewer tier shipped in v0.119.0.)
