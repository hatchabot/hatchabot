...
vector half needs an *embedding provider*. AgentClaw sets
...
- Both paths are exposed as `AGENTCLAW_EMBED_PLUGIN` / `AGENTCLAW_EMBED_MODEL`
  env for discovery.
...
- Build a content revision without an OpenClaw bump:
  ...
  # pin one agent to agentclaw-runtime:2026.7.1-2-emb1, verify memory search
  docker tag agentclaw-runtime:2026.7.1-2-emb1 agentclaw-runtime:latest    # promote
  ```
