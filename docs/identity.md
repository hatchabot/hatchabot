...
Both are token clients. CLI: `hatchabot login` does the REST sign-in, stores
the refresh token in `~/.config/hatchabot/env`, refreshes ID tokens as
...
1. **Seam** — ✅ shipped 2026-08-01. `HATCHABOT_AUTH=password|identity`
   ...
   rotating `HATCHABOT_PASSWORD` invalidates outstanding 30-day cookies.
...
2. **Identity mode** — ✅ shipped 2026-08-01. `src/api/identity.ts` verifies
   ...
   `hatchabot login` stores a refresh token 0600. `/v1/config` (open in both
   modes) tells the login screen which mode to render.
   Configured via env: `HATCHABOT_GCP_PROJECT`,
   `HATCHABOT_IDENTITY_API_KEY`, `HATCHABOT_GOOGLE_CLIENT_ID`.
