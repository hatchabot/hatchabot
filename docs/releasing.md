...
A production AgentClaw should run from a **checkout pinned to a tag**, not
...
```
~/agentclaw        # development: branches, uncommitted work, tests
~/agentclaw-prod   # production: always at a tag; nothing edited by hand
~/agentclaw-data   # state: SQLite (which also holds the encrypted secrets), backups
```
...
WorkingDirectory=%h/agentclaw-prod
EnvironmentFile=%h/agentclaw-prod/.env
ExecStart=%h/agentclaw-prod/node_modules/.bin/tsx src/index.ts
```
and in `.env`: `AGENTCLAW_DB=/home/<you>/agentclaw-data/agentclaw.sqlite`.
Backups already default to `~/agentclaw-backups` (`AGENTCLAW_BACKUP_DIR`), outside
