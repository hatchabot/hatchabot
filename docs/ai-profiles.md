...
difference decides what Hatchabot can host.
...
2. **A subscription is a single person's seat, with per-account rate limits.**
   ...
   they can judge. For Hatchabot hosting agents on their behalf it becomes our
...
(~1 year) token tied to the subscription, which Hatchabot stores encrypted and
...
- **Stored on the agent, not the volume.** The override lives in Hatchabot's DB
...
- **Refused paths** — credential directories (`~/.claude`, `~/.ssh`,
  `~/.config/hatchabot`), system paths (`/etc`, `/root`, `/proc`, `/sys`), the
