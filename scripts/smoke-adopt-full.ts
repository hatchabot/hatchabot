 ...
 *
 * Stands up a THROWAWAY Hatchabot server — its own port, its own temp data dir,
...
const OWNER_HEADER: Record<string, string> = { 'x-hatchabot-owner': 'dev-owner' };
...
async function startServer() {
  step(`Start a throwaway Hatchabot on :${PORT} (docker prefix ${PREFIX})`);
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    HATCHABOT_DB: join(dataDir, 'hatchabot.sqlite'),
    HATCHABOT_SECRET_KEY: randomBytes(32).toString('hex'),
    HATCHABOT_PREFIX: PREFIX,
    ...
    HATCHABOT_IMAGE: process.env.HATCHABOT_IMAGE || 'hatchabot-runtime:latest',
  };
  delete env.HATCHABOT_PASSWORD;
