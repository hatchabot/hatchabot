...
# workspace against the real running AgentClaw, real Docker, and a real Telegram
...
#
#   AGENTCLAW_SMOKE_BOT_TOKEN=<a throwaway BotFather token> ./scripts/smoke-adopt.sh
#
...
cleanup() {
  step "Cleanup"
  agentclaw delete "$NAME" --yes >/dev/null 2>&1 && ok "deleted agent $NAME (container + volume purged)" \
...
# ---- preflight --------------------------------------------------------------
step "Preflight"
command -v agentclaw >/dev/null || die "agentclaw CLI not on PATH"
command -v docker    >/dev/null || die "docker not on PATH"
docker info >/dev/null 2>&1     || die "docker daemon not reachable"
agentclaw list >/dev/null 2>&1  || die "AgentClaw not reachable / not logged in (run: agentclaw login)"
ok "CLI, docker, and a logged-in server are all present"

if [ -z "${AGENTCLAW_SMOKE_BOT_TOKEN:-}" ]; then
  printf '\n\033[33mSKIP\033[0m: set AGENTCLAW_SMOKE_BOT_TOKEN to a throwaway BotFather token to run the live test.\n'
...
cat > "$WS/SOUL.md" <<'MD'
# Smoke Test Agent
A disposable agent that exists only to verify AgentClaw's adopt flow end to end.
...
if ! agentclaw adopt "$WS" "$NAME" --bot-token "$AGENTCLAW_SMOKE_BOT_TOKEN"; then
...
for _ in $(seq 1 45); do
  ...
  state="$(agentclaw list | awk -v n="$NAME" '$1==n {print $2; exit}')"
...
if docker ps --format '{{.Names}}' | grep -q "^agentclaw-${NAME}-"; then
  ok "a Docker container is up: $(docker ps --format '{{.Names}}' | grep "^agentclaw-${NAME}-")"
else
  die "no running Docker container matches agentclaw-${NAME}-*"
fi

# The bot token Telegram actually accepts (soft — network hiccups shouldn't fail).
if agentclaw health "$NAME" >/dev/null 2>&1; then
