...
//   HATCHABOT_AGENT_DISK_WARN_GB   per-agent warn threshold (default 10)
import { execSync } from 'node:child_process';

const warnGB = Number(process.env.HATCHABOT_AGENT_DISK_WARN_GB) || 10;
const sh = (c) => { try { return execSync(c, { stdio: ['ignore', 'pipe', 'ignore'] }).toString(); } catch { return ''; } };

// `docker system df -v` reports each volume's size; parse the hatchabot ones.
