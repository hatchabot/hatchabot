...
chan() { git show origin/main:channels.json 2>/dev/null | sed -nE "s/.*\"$1\"[[:space:]]*:[[:space:]]*\"(v[^\"]+)\".*/\1/p" | sed -n 1p; }
STABLE="$(chan stable)"; BETA="$(chan beta)"
LATEST="$(git tag -l 'v[0-9]*' --sort=-v:refname | grep -vE -- '-(rc|beta|alpha)' | sed -n 1p)"
