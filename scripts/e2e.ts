 ...
 *
 *   HATCHABOT_SECRET_KEY=$(openssl rand -hex 32) npm run e2e
 */
...
// passphrase-style HATCHABOT_SECRET_KEY in the environment doesn't crash the
// e2e with "must be 32 bytes"; default to a fixed hex key when unset.
const secrets = new LocalSecretStore(
  db,
  process.env.HATCHABOT_SECRET_KEY
