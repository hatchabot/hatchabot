...
/**
 * Derived images: an owner builds `FROM agentclaw-runtime:<base>` + their own
...
describe('baseProblem', () => {
  it('requires an agentclaw-runtime tag and refuses chaining', () => {
    expect(baseProblem('agentclaw-runtime:latest')).toBeNull();
    expect(baseProblem('agentclaw-runtime:2026.7.1-2-emb1')).toBeNull();
    expect(baseProblem('ubuntu:22.04')).toMatch(/must be a agentclaw-runtime/i);
    // A derived image built on another derived image can't be rebuilt from a
    // promoted base — refuse the tower.
    expect(baseProblem('agentclaw-runtime:derived-x')).toMatch(/cannot be based on another/i);
  });
});

describe('deriveTag', () => {
  it('namespaces under the base repo so images sort together', () => {
    expect(deriveTag('media')).toBe('agentclaw-runtime:derived-media');
  });
});
...
describe('renderDockerfile', () => {
  it('runs the snippet as root then restores USER node', () => {
    ...
    const out = renderDockerfile('agentclaw-runtime:latest', 'RUN apt-get install -y ffmpeg');
    expect(out).toBe(
      'FROM agentclaw-runtime:latest\nUSER root\nRUN apt-get install -y ffmpeg\nUSER node\n',
    );
...
describe('Store derived_images', () => {
  ...
  it('upserts BUILDING, then flips to READY with a built_at stamp', () => {
    const s = store();
    s.upsertDerivedImage({ name: 'media', tag: deriveTag('media'), base: 'agentclaw-runtime:latest', dockerfile: 'RUN true', createdBy: 'u1' });
  ...
  it('a re-derive under the same name resets to BUILDING and replaces the dockerfile', () => {
    const s = store();
    s.upsertDerivedImage({ name: 'x', tag: deriveTag('x'), base: 'agentclaw-runtime:latest', dockerfile: 'RUN a', createdBy: 'u1' });
    s.setDerivedImageStatus('x', 'FAILED', 'boom');
    s.upsertDerivedImage({ name: 'x', tag: deriveTag('x'), base: 'agentclaw-runtime:latest', dockerfile: 'RUN b', createdBy: 'u1' });
  ...
  it('lists sorted and deletes', () => {
    ...
    for (const n of ['zeta', 'alpha']) {
      s.upsertDerivedImage({ name: n, tag: deriveTag(n), base: 'agentclaw-runtime:latest', dockerfile: 'RUN x', createdBy: 'u1' });
    }
...
const H = (owner: string) => ({ 'x-agentclaw-owner': owner });
...
describe('POST /v1/images', () => {
  it('host owner: creates a BUILDING row and kicks the build', async () => {
    ...
    expect(res.json().tag).toBe('agentclaw-runtime:derived-media');
    const rec = store.getDerivedImage('media')!;
    expect(rec.base).toBe('agentclaw-runtime:latest'); // default base
...
describe('DELETE /v1/images/:name', () => {
  it('refuses while an agent pins the tag, allows once unpinned', async () => {
    ...
    store.setAgentImage('a1', 'agentclaw-runtime:derived-media');
...
describe('GET /v1/images', () => {
  it('lists with pin counts and the fleet base, host-owner only', async () => {
    ...
    const body = res.json();
    expect(body.base).toBe('agentclaw-runtime:latest');
