...
describe('GET/PUT /v1/agents/:id/files/:name', () => {
  ...
  it("another account's agent is a 404, not a read", async () => {
    ...
    const res = await f.inject({ method: 'GET', url: '/v1/agents/a1/files/SOUL.md', headers: { 'x-hatchabot-owner': 'stranger' } });
...
describe('fleet search key routes', () => {
  it('host-owner gated, write-only lifecycle', async () => {
    ...
    const stranger = await f.inject({ method: 'GET', url: '/v1/search-key', headers: { 'x-hatchabot-owner': 'someone-else' } });
...
describe('agent connections (gog accounts)', () => {
  ...
  it('shell-metacharacter emails are refused before any exec; foreign agents 404', async () => {
    ...
    const foreign = await f.inject({ method: 'GET', url: '/v1/agents/a1/connections', headers: { 'x-hatchabot-owner': 'someone-else' } });
...
describe('GET /v1/bot-inventory', () => {
  it('host-owner gated; lists every known bot with a live verdict, no tokens', async () => {
    const { f } = await liveWorld();
    const stranger = await f.inject({ method: 'GET', url: '/v1/bot-inventory', headers: { 'x-hatchabot-owner': 'someone-else' } });
    expect(stranger.statusCode).toBe(403);
