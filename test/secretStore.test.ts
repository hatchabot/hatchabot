...
describe('keyFromEnv', () => {
  it('treats 64 hex chars as the raw key and anything else as a passphrase', () => {
    ...
    const raw = LocalSecretStore.keyFromEnv({ AGENTCLAW_SECRET_KEY: hex } as any);
    expect(raw).toEqual(Buffer.from(hex, 'hex'));
    // 63 chars is not a hex key — it must be stretched, not truncated
    const pass = LocalSecretStore.keyFromEnv({ AGENTCLAW_SECRET_KEY: 'a'.repeat(63) } as any);
  ...
  it('is deterministic — a restart must decrypt what the last run wrote', () => {
    const a = LocalSecretStore.keyFromEnv({ AGENTCLAW_SECRET_KEY: 'my passphrase' } as any);
    const b = LocalSecretStore.keyFromEnv({ AGENTCLAW_SECRET_KEY: 'my passphrase' } as any);
    expect(a).toEqual(b);
  });

  it('refuses to start without a key', () => {
    expect(() => LocalSecretStore.keyFromEnv({} as any)).toThrow(/AGENTCLAW_SECRET_KEY/);
  });
});
