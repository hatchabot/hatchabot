 ...
 * AGENTCLAW_BACKUP_DIR; the tarball name matches MockProvider's volume archive.
 */
...
const prevEnv = process.env.AGENTCLAW_BACKUP_DIR;
beforeAll(() => { process.env.AGENTCLAW_BACKUP_DIR = base; });
afterAll(() => {
  if (prevEnv === undefined) delete process.env.AGENTCLAW_BACKUP_DIR;
  else process.env.AGENTCLAW_BACKUP_DIR = prevEnv;
...
function writeSet(date: string, w: World, agentIds: string[], opts: { db?: boolean; key?: boolean } = {}) {
  ...
  if (opts.db !== false) writeFileSync(join(dir, 'agentclaw.sqlite'), 'db');
