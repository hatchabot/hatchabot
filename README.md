...
| `HATCHABOT_EMBED_PORT`, `HATCHABOT_EMBED_BIND`, `HATCHABOT_EMBED_PER_MIN` | The machine's embedding service for agents' semantic memory search (Settings → Hosts): the door's port (`8093`), the address it is published on (the docker bridge gateway), calls a minute per agent (`600`). `HATCHABOT_EMBED_URL` (with `HATCHABOT_EMBED_KEY`, `HATCHABOT_EMBED_MODEL`) points switched agents at a server you already run instead, and starts no container. `HATCHABOT_EMBEDDER_MEMORY` (`1g`), `HATCHABOT_EMBEDDER_IMAGE`, `HATCHABOT_EMBED_HEALTH_MS` (5 min) tune the service. |
