...
| `HATCHABOT_AGENT_MEMORY` | Per-agent container memory cap, default `3g` (was `2g` before OpenClaw 2026.9, whose gateway idles near 1 GB). Applies to containers made after the change: Rebuild to apply. Status → Resources shows each container's peak and how often it hit its cap. |
