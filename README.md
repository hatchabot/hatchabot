...
| `HATCHABOT_AUTH` | `accounts` (what the installer writes), `password` (the fallback when unset) or `identity` — see below. |
...
| `HATCHABOT_HIBERNATE_AFTER` | Put an agent to sleep (stopped, volume kept) after this long without activity — `90m`, `6h`, `2d`; unset = never. A Telegram message, opening its console, or `hatchabot ask` wakes it; Discord/Slack agents and agents with scheduled tasks are never put to sleep. Per agent: `PATCH … {"hibernate":"never"}` or `hatchabot hibernate/wake`. |
| `HATCHABOT_HOST_ALIAS_IP`, `HATCHABOT_DOCKER_ROOTLESS` | Rootless Docker is detected (`docker info`) and handled: agents are probed on their published ports and reach this machine at `10.0.2.2` (slirp4netns, with host loopback on). These override the address and the detection. See docs/shared-host.md. |
| `HATCHABOT_YES`, `HATCHABOT_SETUP_SIGNIN`, `HATCHABOT_SETUP_PASSWORD`, `HATCHABOT_SETUP_PORT`, `HATCHABOT_SETUP_ENV` | Unattended install: `HATCHABOT_YES=1` answers the installer's offers, the `SETUP_*` values answer setup-host.sh (sign-in `accounts`/`password`, the port, extra .env lines). |
...
| `HATCHABOT_AGENT_MEMORY` | The fleet-default memory cap on an agent's container, default `3g` (was `2g` before OpenClaw 2026.9, whose gateway idles near 1 GB). A class or an agent can carry its own cap (agent sheet → Runtime → Memory cap; `hatchabot memory`), applied right away and kept across rebuilds. Status → Resources shows each container's peak and how often it hit its cap. |
| `HATCHABOT_AGENT_MEMORY_MAX` | The most a member may give one of their agents, default `8g` (never below the default). The machine's owner is not bound by it. |
| `HATCHABOT_REBUILD_CONCURRENCY` | How many rebuilds run at once (1–12), default `6`; also set from Settings → Images → Rebuild at once, or `hatchabot rebuild-policy --at-once N`. Beyond it, rebuilds wait their turn. |
| `HATCHABOT_FILE_MAX_MB` | Largest single download or upload through an agent's Files tab (a file, or a folder as `.tar.gz`), default `512`. |
| `HATCHABOT_REBUILD_POLICY` | What the machine rebuilds on its own: `required-only` (default), `auto` (recommended ones too, in `HATCHABOT_REBUILD_QUIET_HOURS`, default `3-5`), or `manual`. Settings → Images → Automatic rebuilds. |
| `HATCHABOT_IMPORT_MAX_GB` | How much an imported `.hatchabot` file may expand to on the volume, default `8`. |
| `HATCHABOT_EMBED_DEFAULT` | Which memory search engine new agents get: `baked` (their own, default) or `shared` (the machine's service). Status → Tools sets it. |
| `HATCHABOT_EMBED_PORT`, `HATCHABOT_EMBED_BIND`, `HATCHABOT_EMBED_PER_MIN` | The machine's embedding service for agents' semantic memory search (Settings → Hosts): the door's port (`8093`), the address it is published on (the docker bridge gateway), calls a minute per agent (`600`). `HATCHABOT_EMBED_URL` (with `HATCHABOT_EMBED_KEY`, `HATCHABOT_EMBED_MODEL`) points switched agents at a server you already run instead, and starts no container. `HATCHABOT_EMBEDDER_MEMORY` (`2g`), `HATCHABOT_EMBEDDER_IMAGE`, `HATCHABOT_EMBED_HEALTH_MS` (5 min) tune the service. |
| `HATCHABOT_ALLOW_MACHINE_LOGIN` | Set to `1` to allow new Claude sources that mount this machine's `~/.claude` into agents (off since v2.39.0; use `claude setup-token`). |
...
| `HATCHABOT_OPS_PORT` | Port of the small server the management agent talks to (its tools and its filtered route to its AI provider), default `8091`. It listens on loopback or the docker bridge gateway (`HATCHABOT_OPS_BIND`), reached only through the agent's doorman container. |
...
- **Agents are powerful, and that is the design.** Each runs OpenClaw (with
  Claude Code inside) with permission prompts disabled, a shell, and
  unrestricted outbound internet. A setup-token or API-key source injects a
  token and mounts nothing. The older *machine login* source bind-mounted your
  `~/.claude` read-write into that owner's agents; new sources of that kind can
  no longer be made (v2.39.0) — use a setup token.
