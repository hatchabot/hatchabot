...
| `HATCHABOT_AUTH` | `password` (default) or `identity` — see below. |
...
| `HATCHABOT_BIND` | Listen address. Defaults to `0.0.0.0`, or `127.0.0.1` when no password is set. |
...
**Authentication.** By default Hatchabot uses one shared password — right for a
home install on a private network. Set `HATCHABOT_AUTH=identity` to use real
per-user accounts via GCP Identity Platform (Google sign-in and/or
email/password) instead; that path needs `HATCHABOT_GCP_PROJECT`,
`HATCHABOT_IDENTITY_API_KEY`, and `HATCHABOT_GOOGLE_CLIENT_ID`. Setup and
