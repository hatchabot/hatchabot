...
| `AGENTCLAW_TLS_CERT` / `AGENTCLAW_TLS_KEY` | Paths to a PEM cert and private key. Set **both** to serve HTTPS directly (no reverse proxy); setting only one fails loudly. |
| `AGENTCLAW_MAX_AGENTS_PER_ACCOUNT` | Optional per-account cap on how many agents one account may create. Unset (or `0`) means no limit. |
